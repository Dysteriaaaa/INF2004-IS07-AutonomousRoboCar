...
`docs/img/hw/robopico_pin_map.png` draws every signal onto the physical Robo
Pico board (Grove 1 left edge, Grove 7 right edge, Grove 2-6 bottom, motor
terminals and servo header top). It is generated by
`docs/img/hw/gen_pin_map.py` and mirrored to
`documents/week6_diagrams/d2_pin_layout.png` for the design review; edit the
script, never the PNG. Board geometry came from the Cytron Robo Pico
datasheet in `documents/Robo_Pico.pdf`; signal assignments come from
`core/rc_config.h` and section 2 of the Week 6 design review. Note that the
Robo Pico silkscreen carries **GP26 on both Grove 5 and Grove 6**.
