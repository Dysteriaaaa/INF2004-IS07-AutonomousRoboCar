...
The diagram above shows the same assignments laid out on the physical board:
Grove 1 on the left edge, Grove 7 on the right edge, Grove 2–6 along the
bottom, motor terminals and the four-way servo header along the top. It is
generated by `docs/img/hw/gen_pin_map.py` — edit that script rather than the
PNG, and re-run it to regenerate. A copy for the design-review document is
written to `documents/week6_diagrams/d2_pin_layout.png`.
